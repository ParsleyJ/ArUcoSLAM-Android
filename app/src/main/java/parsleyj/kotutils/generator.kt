...
/*
   JS ES6-style generator that can send values between coroutine and outer code in both ways.
   ...
   we must declare `next` to be always invoked with a parameter and make our coroutine receive the
   first parameter to `next` when it starts (so it is not lost). We also have to introduce an
   additional parameter to `yieldAll` to start a delegated generator.
*/
...
fun <T> iterate(g: Generator<T, Unit>) = object : Iterable<T> {
    override fun iterator(): Iterator<T> {
        return object : Iterator<T> {
            private var cachedNext: T? = null

            override fun hasNext(): Boolean {
                cachedNext = g.next(Unit)
